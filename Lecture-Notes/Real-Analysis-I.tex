...
\end{definition}
\begin{example}
	Open ball in $\mb R^k$: $B_r(\vx) := \{ \vy\in\mb R^k : |\vx-\vy|<r \}$ - ball centered at $\vx$ with radius $r>0$.
	
	\ul{Claim}: $B_r(\vx)$ is convex.
	\begin{proof}
		Let $\vy,\vz\in B_r(\vx)$ and $\lambda\in[0,1]$.
		Want to show: $\lambda\vy + (1-\lambda)\vx\in B_r(\vx)$.
		\begin{align*}
			|\lambda\vy + (1-\lambda)\vz - \vx|_{\mb R^k} &\os{?}{<} r \\
														  &= | \lambda\vy + \vz - \lambda\vz - \vx| \\
						&= |\lambda\vy -\lambda\vx +\lambda\vx +\vz -\lambda\vz -\vx|
		\end{align*}
	\end{proof}
\end{example}
