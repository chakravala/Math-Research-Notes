...
\begin{proof}
	...
	There exists $y_0\in Y$ such that $a<||v-y_0||\leq \frac a\theta$.
	...
	\begin{align*}
		||z-y|| &= ||c(v-y_0)-y||
				...
				= c||v-y_1||,
	\end{align*}
	where $y_1 = y_0+\frac1cy\in Y$, so $y_1\in Y$.
	So
		$||z-y|| = c||v-y_1|| 
				\geq c\cdot a 
				= \frac a{||v-y_0||} 
...
\end{definition}
\begin{definition}
	The \ul{null space} of $T$, denoted $N(T)$, is $N(T) = \setc{x\in \ms D(T)}{Tx=0}$.
\end{definition}
\begin{theorem}
