...
\ul{Assumption} $x_n\ra\alpha$ as $n\ra\infty$. \ul{IF} the method converges, \ul{THEN} we can detect when $|\us{\text{error}}{x_N-\alpha}|<\text{TOL}$ and stop searching.

\ul{Assumption} $f'(\alpha)\neq0$ and $f,f'$ are continuous on $[a,b]$ with $a<\alpha<b$.
\begin{enumerate}
	\item[$*$] we can choose an interval $[\tilde a,\tilde b]$ such that
		...
		\end{itemize}
\end{enumerate}
\ul{Idea}: By the MVT, $\exists \xi_n$ between $x_n$ and $\alpha$ such that $f(x_n)-f(\alpha) = f'(\xi_n)(x_n-\alpha) \implies \fbox{$(x_n-\alpha) = \frac{f(x_n)}{f'(x_n)}$}$

\ul{Case 1}: $|f(x_n)|$ small
\begin{align*}
	|x_n-\alpha| &= \frac{|f(x_n)|}{|f'(x_n)|} \leq \frac{|f(x_n)|}{\min_{\tilde a\leq\xi\leq\tilde b}|f'(\xi)|} \\
				 &\leq \frac{|f(x_n)|}m = \frac1m|f(x_n)|\ <\text{TOL}
\end{align*}
Typicall $<5\text{TOL}$. This implies STOP at $N$ if $|f(x_N)|\leq m\text{TOL}$.
